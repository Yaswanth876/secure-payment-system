...
export default function DemoAuthorization({ preview, loading, error, onBack, onAuthorize }) {
  ...
  return <section className="page reveal amount-confirmation-page">
    <div className="page-header"><button className="back-button" onClick={onBack} aria-label="Back to amount confirmation">Back</button><h1>Confirm payment</h1></div>
    <p className="page-intro">Demo authorization only. Do not enter a real UPI PIN.</p>
    <div className="receipt-person amount-confirmation-recipient"><span className="avatar">{preview.recipient.photo ? <img src={preview.recipient.photo} alt="" /> : preview.recipient.name.slice(0, 1).toUpperCase()}</span><div><span className="eyebrow">Paying</span><h2>{preview.recipient.name}</h2><p>{preview.recipient.upiId}</p><strong>{money(preview.amount.rupees)}</strong></div></div>
    <form onSubmit={submit}><label className="pin-field"><span>Demo PIN</span><input autoFocus type="password" inputMode="numeric" autoComplete="off" maxLength="6" value={pin} onChange={event => setPin(event.target.value.replace(/\D/g, ''))} aria-describedby="pin-help" aria-label="Demo PIN, at least four digits" /></label><p id="pin-help" className="field-note">Use any four to six digits for this simulated step. It is not stored or sent to the backend.</p>{error && <p className="inline-error" role="alert">Payment authorization failed. {error}</p>}{loading ? <div className="inline-loading"><span className="spinner" /> Authorizing payment</div> : <button className="primary-button wide" type="submit" disabled={!ready}>Pay {money(preview.amount.rupees)}</button>}</form>
  </section>;
}
