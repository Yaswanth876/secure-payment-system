...
function Home({ profile, transactions, onPay, onActivity, onOpen }) { return <section className="page reveal"><div className="eyebrow">Payment safety, made clear</div><h1>Good evening,<br /><em>{profile.name}</em></h1><div className="hero-action"><div><span className="eyebrow">Ready when you are</span><h2>Make a payment</h2><p>Review who, how much, and where it comes from.</p></div><button className="primary-button" onClick={onPay}>Pay <span>→</span></button></div><div className="section-heading"><div><span className="eyebrow">Your latest payments</span><h2>Recent activity</h2></div><button className="text-button" onClick={onActivity}>See all <span>→</span></button></div>{transactions.slice(0, 3).map(transaction => <TransactionCard key={transaction.transactionId} transaction={transaction} onClick={() => onOpen(transaction.transactionId)} />)}<div className="principle"><span className="principle-number">01</span><div><strong>Know before you confirm</strong><p>Every payment shows the recipient, exact amount, and account before it moves.</p></div></div></section>; }
...
function PageHeader({ title, onBack }) { return <div className="page-header"><button className="back-button" onClick={onBack} aria-label="Go back">←</button><h1>{title}</h1></div>; }
