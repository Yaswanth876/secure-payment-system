...
export default function SafetyHold({ intent, initialState = 'HOLD_REQUIRED', onVerified }) {
  ...
  return <Card variant="outlined" className={`safety-hold safety-hold--${state.toLowerCase()}`}><p className="eyebrow">Demo safety workflow</p><StatusIndicator status={state === 'HOLD_VERIFIED' ? 'success' : state === 'HOLD_CANCELLED' ? 'neutral' : 'warning'}>{state.replace('_', ' ')}</StatusIndicator><h2>{title}</h2><p>{message}</p><div className="safety-hold__summary"><strong>₹{formatIndianAmount(intent.amount)}</strong><span>{intent.recipient.name}</span></div>{state === 'HOLD_REQUIRED' && <Button fullWidth onClick={() => setState('HOLD_ACTIVE')}>Verify</Button>}{state === 'HOLD_ACTIVE' && <div className="safety-hold__actions"><Button onClick={() => { setState('HOLD_VERIFIED'); onVerified?.() }}>Complete demo verification</Button><Button variant="outline" onClick={() => setState('HOLD_CANCELLED')}>Cancel</Button></div>}</Card>
}
