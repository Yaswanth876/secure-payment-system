...
export default function Send() {
	...
	const continueFromReceipt = async () => { if (!intent || isPreparing) return; setIsPreparing(true); setApiError(''); try { const nextDecision = await getSafetyDecision({ recipientId: intent.recipientId, amount: intent.amount, recipient: intent.recipient }); setStep(nextDecision.requiresSafetyHold ? 'hold' : nextDecision.requiresCoolingOff ? 'cooling' : 'slider') } catch { setApiError("We couldn't check the payment safety details. Please try again.") } finally { setIsPreparing(false) } }
